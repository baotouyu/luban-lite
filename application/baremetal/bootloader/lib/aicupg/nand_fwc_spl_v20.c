...
struct nand_page_table_head {
	...
	u8  pad; /* Padding it to fit size 20 bytes */
};
...
struct aicupg_nand_spl {
    struct mtd_dev *mtd;
    u32 spl_blocks[SPL_NAND_GOOD_BLOCKS_NUM];
    u32 rx_size;
    u32 buf_size;
    u8 *image_buf;
	u32 spienc_tweak;
...
{
    u32 spl_blocks[SPL_NAND_GOOD_BLOCKS_NUM];
    u32 block_mark[SPL_NAND_GOOD_BLOCKS_NUM];
    struct mtd_dev *mtd;
    s32 ret, i;
...
static s32 nand_spl_erase_image_blocks(struct aicupg_nand_spl *spl)
{
    ...
    mtd = spl->mtd;
    for (i = 0; i < SPL_NAND_GOOD_BLOCKS_NUM; i++) {
...
{
    u32 pcnt, pgidx, data_size, i, sumval, pa = 0, blkidx;
    u32 slice_size;
    u8 *page_data, *p, *end;
    s32 ret = 0;
	...
	pt->head.pages_per_block = spl->mtd->erasesize / spl->mtd->writesize;
	pt->head.blocks = spl->mtd->size / spl->mtd->erasesize;
	/* pt.head.planes = mtd_to_nanddev(spl->mtd)->memorg.planes_per_lun;; */
	pt->head.planes = 1;
    ...
    for (pgidx = 1; pgidx < pcnt + 1; pgidx++) {
		...
		for (i = 0; i < SPL_NAND_IMAGE_BACKUP_NUM; i++) {
            if (pgidx < PAGE_CNT_PER_BLOCK)
			    blkidx = spl->spl_blocks[2 * i];
            else
			    blkidx = spl->spl_blocks[2 * i + 1];
			if (blkidx == SPL_INVALID_BLOCK_IDX) {
				pr_debug("%s, block id is invalid\n", __func__);
				continue;
			}
            if (pgidx < PAGE_CNT_PER_BLOCK)
			    pa = (blkidx << 6) + pgidx;
            else
			    pa = (blkidx << 6) + pgidx - PAGE_CNT_PER_BLOCK;
	...
	memcpy(page_data, (u8 *)pt, sizeof(struct nand_page_table));
...
static s32 nand_fwc_spl_program(struct fwc_info *fwc, struct aicupg_nand_spl *spl)
{
    ...
    for (pgidx = 1; pgidx < pt->head.entry_cnt; pgidx++) {
		...
		for (i = 0; i < SPL_NAND_IMAGE_BACKUP_NUM; i++) {
            ...
            offset = pa * spl->mtd->writesize;
            pr_info("Write data to blk %d pa 0x%x, offset 0x%x\n", blkidx, pa, (u32)offset);
                             ...
                             struct nand_page_table *pt)
{
    ...
    for (i = 1; i < pt->head.entry_cnt; i++) {
        ...
        /*     cksum = pt->entry[i % PAGE_TABLE_MAX_ENTRY].checksum; */
        /* } */
